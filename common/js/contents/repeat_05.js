...
//			_disX		= Math.abs(_x - this.mousePoint.x),
//			_disY		= Math.abs(_y - this.mousePoint.y),
			_distance	= LIB.getPointDistance({x:_x,y:_y},this.mousePoint),
			_range		= 500,
			_level		= (_range / _distance * 10 | 0) / 10,
			_isAreaIn	= (_distance < _range) ? true : false ;

		//自分の位置からマウス座標を反発させる
//		if(_isAreaIn){
//			_level	= (_level === 0) ? _range : _level;
//			_level	= (_level > 100) ? 100 : _level;
//			_disX	= (1 - _disX / _range) * _level | 0;
//			_disY	= (1 - _disY / _range) * _level | 0;
//			_disX	= (_x > this.mousePoint.x)? _disX : -_disX;
//			_disY	= (_y > this.mousePoint.y)? _disY : -_disY;
////			console.log(_disX,_disY,_level);
//		}else{
//			_disX = 0;
//			_disY = 0;
//		}
		
		_radius = this.radius + (_range - _distance)*0.06;
		_radius = (_radius < this.radius)? this.radius :_radius;
...
//		else _y -= _moveNoise;
		...
		_c.beginPath();
		_c.fillStyle = (_isAreaIn)? this.fillColor_02 : this.fillColor;
